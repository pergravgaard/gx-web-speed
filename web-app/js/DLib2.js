...
//					}
	...
	event: {
		...
		listeners: [],
        xEvents: {
            'x-pointerdown': 'pointerdown mousedown touchstart',
            'x-pointermove': 'pointermove mousemove touchmove',
            'x-pointerup': 'pointerup mouseup touchend'
        },
        isXEvent: function(/* String */ eventType) {
            return (eventType in this.xEvents);
        },
        ...
        xHandler: function(eventTypes, f, doCapture) {
            var xfn = function(e) {
                if (!xfn.done) {
                    for (var i = 0, l = eventTypes.length; i < l; i++) {
                        var et = eventTypes[i];
                        if (e.type !== et) {
                            dLib.event.remove(this, et, xfn, doCapture);
                        }
                    }
                    xfn.done = true;
                }
            ...
            xfn.bound = f.bound; // make sure that the add method doesn't bind twice
            return xfn;
        },

        addAll: function(target, eventTypes, handler, doCapture) {
            var isXEvent = this.isXEvent(eventTypes);
            if (isXEvent) {
                dLib.assertType(handler, 'function', 'When using an x-event type you must specify a function as listener!');
                eventTypes = this.xEvents[eventTypes];
            }
            var types = eventTypes.trim().split(/\s+/);
            if (isXEvent) {
                handler = this.xHandler(types, handler, doCapture);
            }
        ...
        deleteAll: function(target, eventTypes, handler, doCapture) {
            if (this.isXEvent(eventTypes)) {
                eventTypes = this.xEvents[eventTypes];
            }
            var types = eventTypes.trim().split(/\s+/);
            for (var i = 0, l = types.length; i < l; i++) {
                if (Object.isObject(handler) || typeof handler === 'function') {
                    this.remove(target, types[i], handler, doCapture);
                } else {
                    this.removeAll(target, types[i], handler);
                }
            }
        },
	...
	/**	Cross-browser method to add an onDOMReady listener.
		...
		addDOMReady: function(listener) {
			...
			if (document.addEventListener) {
				...
				document.attachEvent("onreadystatechange", function anonComplete() {
					if (document.readyState === "complete") {
						document.detachEvent("onreadystatechange", anonComplete);
...
DDocument.prototype.addEventListener = function(eventTypes, handler, doCapture) {
	return dLib.event.addAll(this.document, eventTypes, handler, doCapture);
}

DDocument.prototype.on = function(eventTypes, handler, doCapture) {
    this.addEventListener(eventTypes, handler, doCapture);
	return this;
}

DDocument.prototype.removeEventListener = function(eventTypes, handler, doCapture) {
	dLib.event.deleteAll(this.document, eventTypes, handler, doCapture);
}

DDocument.prototype.off = function(eventTypes, handler, doCapture) {
	this.removeEventListener(eventTypes, handler, doCapture);
	return this;
}
...
DElement.prototype.addEventListener = function(eventTypes, handler, doCapture) {
	return dLib.event.addAll(this.element, eventTypes, handler, doCapture);
}
...
DElement.prototype.removeEventListener = function(eventTypes, handler, doCapture) {
	dLib.event.deleteAll(this.element, eventTypes, handler, doCapture);
}
