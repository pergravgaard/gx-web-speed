// TODO: add support for mouseenter and mouseleave (they are not triggered by child elements and do not propagate/bubble).
// Also add support for tap event on mobile devices.
// Make DTimer use requestAnimationFrame
// Drop support for IE 8 and below
...
var dLib = {
	...
	event: {
		readyListeners: [],
		listeners: [],
		w3cOnly: false, // if true the event wrapper class won't be used and the old MS DOM Event Model won't be supported
		isDOMReady: false,
	...
	util: {
		...
		fireHandler: function(name, args, cfg) {
			args = args || [];
			if (!args.push && typeof args.length === 'number') { // allows us to specify arguments as the args object
				args = [].addAll(args);
			}
			args.push(this);
			cfg = cfg || this.config || this;
	...
	get: function(obj, config, win) {
		...
		if (obj) {
			if (!(obj instanceof DNode) && typeof obj.nodeName == "string") { // IE5.5 does not support the nodeName property for the document object
				...
				if (obj.srcElement && eventType.indexOf('MS') == 0) {
					eventType = eventType.replace('MS', '').toLowerCase();
				}
				switch (eventType) {
					case 'keydown':
					case 'keypress':
	...
	if (!isNative(Array.prototype, "forEach")) {
		Array.prototype.forEach = function(fn, thisObj) {
			...
			for (var i = 0, l = this.length; i < l; i++) {
				if (i in this) { // do as what is done in polyfill for Array.prototype.forEach (suggested by MDN)
...
// return true if all function props in source are present in target
Object.implements = function(target, source) {
	if (!target || !source || typeof target !== 'object' || typeof source !== 'object') {
		return false;
	}
	for (var property in source) {
		if (typeof source[property] === 'function' && typeof target[property] !== 'function') {
			return false;
		}
	}
	return true;
...
Array.prototype.addAll = function(/* list1, list2, list3, ... */) {
...
Boolean.parse = function(v) {
	if (typeof v == 'string') {
		...
		if (lv == 'undefined' || lv == 'null' || lv == 'false') {
			return false;
		}
	}
	return !!v;
...
Number.isNumber = function(n) {
	return typeof n == 'number' && isFinite(n); // returns true for every number greater than or equal to Number.MIN_VALUE and smaller than or equal to Number.MAX_VALUE (including 0). NaN, undefined, null and the empty string will result in false.
}
...
DEvent.stopImmediatePropagation = function(e) {
...
DEvent.prototype.stopImmediatePropagation = function() {
	DEvent.stopImmediatePropagation(this.event);
	return this;
}
...
function DPointerEvent(event) {
	DPointerEvent.superConstructor.apply(this, [event]);
}

DPointerEvent.inheritFrom(DEvent);

DPointerEvent.isTouchSupported = function() {
	return 'ontouchstart' in window;
}

DPointerEvent.isMouseEvent = function(e) {
	return e.type.startsWith('mouse') || e.pointerType === 'mouse' || e.pointerType === 4;
}

DPointerEvent.isTouchEvent = function(e) {
	return !!e.touches || e.pointerType === 'touch' || e.pointerType === 2;
}

DPointerEvent.isOneTouchEvent = function(e) {
	return !DPointerEvent.isMultiTouchEvent(e);
}

DPointerEvent.isMultiTouchEvent = function(e) {
	//out('pointer type: '+e.pointerType);
...
DPointerEvent.prototype.isMouseEvent = function() {
	return DPointerEvent.isMouseEvent(this.event);
}

DPointerEvent.prototype.isTouchEvent = function() {
	return DPointerEvent.isTouchEvent(this.event);
}

DPointerEvent.prototype.isOneTouchEvent = function() {
	return DPointerEvent.isOneTouchEvent(this.event);
}

DPointerEvent.prototype.isMultiTouchEvent = function() {
	return DPointerEvent.isMultiTouchEvent(this.event);
}

// TODO: Should be removed/replaced by DPointerEvent
function DMouseEvent(event) {
	DMouseEvent.superConstructor.apply(this, [event]);
}

DMouseEvent.inheritFrom(DPointerEvent);
...
DElement.requestFullScreen = function(elementOrCallback, callback) {
	var element = elementOrCallback || document.documentElement;
	var execute = true;
	if (callback === undefined) {
		if (typeof elementOrCallback === 'function') {
			callback = elementOrCallback;
			element = document.documentElement;
		}
		else if (typeof elementOrCallback == 'boolean') {
			element = document.documentElement;
			execute = false;
		}
	}
	if (callback) {
		['change', 'error'].forEach(function (eventType) {
			['', 'moz', 'webkit', 'ms', 'o'].some(function (prefix) {
				var eventName = 'on' + prefix + 'fullscreen' + eventType;
...
DElement.supportsFullScreen = function() {
	return DElement.requestFullScreen(false);
}

DElement.exitFullScreen = function() {
	var methodNames = ['exitFullscreen', 'mozCancelFullScreen', 'webkitExitFullscreen', 'webkitExitFullScreen'];
	for (var i = 0, l = methodNames.length; i < l; i++) {
		var method = methodNames[i];
...
DElement.isFullScreen = function() {
	return !!(document.fullscreenElement || document.mozFullScreenElement || document.webkitFullscreenElement || document.msFullscreenElement);
}

DElement.toggleFullScreen = function(element, callback) {
	if (DElement.isFullScreen()) {
		DElement.exitFullScreen();
	} else {
		DElement.requestFullScreen(element, callback);
	}
}

DElement.css = function(element, name, value, prefix) {
	function set(el, n, v) {
		switch (n) {
			case "opacity":
			...
			default:
				...
				if (prefix) { // props that needs prefixing, do not need to be translated
					var cName = n.capitalize();
					// Note: In webkit you can use webkit and Webkit prefix, but it uses the lowercase variant internally: try to set WebkitTransition and inspect the style object; you'll see that webkitTransition and transition are set, but not WebkitTransition
					['Moz', 'webkit', 'O', 'ms'].forEach(function(pr) {
						var pName = pr + cName;
						if (pName in el.style) {
							el.style[pName] = v;
						}
					})
				}
	...
	if (name) {
		if (typeof name == "object") {
			prefix = value; // second argument is then the prefix
			for (var p in name) {
				set(element, dLib.util.translateCSSProperty(p), name[p]);
			}
		} else {
			if (value === undefined) {
				var pName = dLib.util.translateCSSProperty(name);
				if (pName in element.style) {
					return element.style[pName];
				}
				var style = DElement.getComputedStyle(element);
				return style ? style[pName] : '';
			}
...
DElement.append = function(el, htm) {
	...
	pEl.innerHTML = htm;
	while (pEl.childNodes.length) {
		// when removing a node from one structure to another it is automatically removed from the original structure
		el.appendChild(pEl.childNodes[0]);
	}
}
...
DElement.getId = function(el) {
	...
	if (!id) {
		...
		id = (n + Math.random() * Date.now()).replace('.', '');
...
DElement.addClass = function(element, newClass, insertFirst) {
	...
	if (element.classList && !insertFirst) {
...
DElement.toggleClass = function(element, className, insertFirst) {
	if (!className) {
		return;
	}
	if (element.classList && !insertFirst) {
	...
	if (DElement.hasClass(element, className)) {
		DElement.removeClass(element, className);
	} else {
		DElement.addClass(element, className, insertFirst);
	}
}
...
DElement.getPageOffset = function(element, adjustForScroll, useParent) {
	var x = 0, y = 0;
	if (!useParent && typeof element.getBoundingClientRect !== 'undefined') {
...
DElement.getOffsetFromParent = function(element, parent) {
	var x = 0, y = 0;
	for (var el = element; el != parent; el = el.offsetParent) {
		x += el.offsetLeft || 0;
		y += el.offsetTop || 0;
	}
	return [x, y];
...
// TODO: use rAF which does not rely on a duration, but only passes a timestamp (as opposed to elapsed time in percent)
...
DElement.animate = function(el, config) {
	...
	cfg.onSchedule = function(timer) {
		...
		var c = timer.config;
		var dt = Math.min((performance.now() - timer.startTime) / c.duration, 1);
...
DElement.toggle = function(element) {
	if (DElement.isDisplayed(element)) {
		DElement.hide(element);
	} else {
		DElement.show(element);
	}
}
...
DElement.prototype.query = function(query) {
	return new DElementList(query, this.element);
}

DElement.prototype.q = DElement.prototype.query;
...
DElement.prototype.insertIframe = function() {
	...
	ifr.setAttribute("id", "iframe_" + (this.element.id || Date.now()));
...
DElement.prototype.removeClass = function(className) {
	DElement.removeClass(this.element, className);
	return this;
}

DElement.prototype.toggleClass = function(className) {
	DElement.toggleClass(this.element, className);
	return this;
}
...
DElement.prototype.toggle = function() {
	DElement.toggle(this.element);
	return this;
}

DElement.prototype.css = function(property, value, prefix) {
	var rv = DElement.css(this.element, property, value, prefix);
	return arguments.length == 1 && !Object.isObject(property) ? rv : this;
}
...
DElement.prototype.rotate = function(angleInDegress) {
	DElement.css(this.element, 'transform', 'rotate(' + angleInDegress + 'deg)', true);
	return this;
}
...
DElement.prototype.fadeOut = function(config) {
	var cfg = Object.extend({ scope: this, transition: "reverse", from: NaN, to: 0, show: false, hide: true, display: 'none', done: dLib.noop }, config);
	cfg.anim = function(dt, cfg) {
		this.setOpacity(Math.max(dt, 0));
		return (dt > cfg.to);
	};
	cfg.callback = function(el, cfg) {
		this.setOpacity(cfg.to);
		if (cfg.hide) { // auto hide when finished fading out - if we don't SELECT elements will still be active for the user (in IE6 even visible)
			this.hide(cfg.display);
		}
		dLib.util.fireHandler('done', null, cfg);
	};
...
DElement.prototype.fadeIn = function(config) {
	var cfg = Object.extend({ scope: this, transition: 'linear', from: 0, to: 1, show: true, display: 'block', done: dLib.noop }, config);
	cfg.anim = function(dt, cfg) {
...
DElement.prototype.attr = function(name, value) {
	...
	return arguments.length == 1 ? rv : this;
}
...
DElementList.forEach = function(htmlCol, fn, scope, asDElement) {
	...
	if (htmlCol && htmlCol.length) {
		for (var i = 0, l = htmlCol.length; i < l; i++) { // Note that htmlCol is a live nodelist - due to performance we do not query length on each iteration (too expensive)
			if (i in htmlCol) { // do as what is done in Array.prototype.forEach - suggested by MDN
				var el = htmlCol[i];
...
DElementList.prototype.remove = function() {
	for (var i = 0; i < this.elements.length; i++) {
		DNode.remove(this.elements[i]);
	};
	return this;
}

DElementList.prototype.css = function(name, value, prefix) {
	var isGetter = arguments.length == 1 && typeof name == 'string';
	var result = isGetter ? [] : null;
	for (var i = 0, l = this.elements.length; i < l; i++) {
		var rv = DElement.css(this.elements[i], name, value, prefix);
		if (result) {
			result.push(rv);
		}
	}
...
DElementList.prototype.hasClass = function(className) {
	for (var i = 0, l = this.elements.length; i < l; i++) {
		if (DElement.hasClass(this.elements[i], className)) {
			return true;
		}
	}
	return false;
...
DElementList.prototype.replaceClass = function(oldClass, newClass, appendIfNotFound) {
	for (var i = 0, l = this.elements.length; i < l; i++) {
		DElement.replaceClass(this.elements[i], oldClass, newClass, appendIfNotFound);
	}
	return this;
}

DElementList.prototype.removeClass = function(className) {
	for (var i = 0, l = this.elements.length; i < l; i++) {
		DElement.removeClass(this.elements[i], className);
	}
	return this;
}

DElementList.prototype.toggleClass = function(className) {
	for (var i = 0, l = this.elements.length; i < l; i++) {
		DElement.toggleClass(this.elements[i], className);
	}
	return this;
}

DElementList.prototype.html = function(htm, append, before) {
	var values = htm === undefined ? [] : null;
	for (var i = 0, l = this.elements.length; i < l; i++) {
		var rv = DElement.html(this.elements[i], htm, append, before);
		if (values) {
			values.push(rv);
		}
	}
...
DElementList.prototype.toggle = function() {
	for (var i = 0, l = this.elements.length; i < l; i++) {
		DElement.toggle(this.elements[i]);
	}
	return this;
}

DElementList.prototype.find = function(query) {
	var result = [];
	for (var i = 0, l = this.elements.length; i < l; i++) {
		var res = DElementList.query(query, this.elements[i]);
	...
	return new DElementList(result);
}
...
DElementList.prototype.attr = function(name, value) {
	var values = value === undefined ? [] : null;
	this.forEach(function(entry) {
...
DElementList.prototype.text = function(value) {
	var values = value === undefined ? [] : null;
	this.forEach(function(entry) {
		var rv = DElement.text(entry, value);
		if (values) {
			values.push(rv);
		}
	});
...
function DQueue(name, config) {
	this.name = name;
	this.config = Object.configure(config || {}, this.constructor.defaultConfig);
	this.queue = [];
}

DQueue.defaultConfig = {
	fifo: true, // fifo means first-in-first-out. If false lifo, which means last-in-first-out
	limit: NaN,
	replaceLastIfLimitExceeded: false,
	throwErrorIfIndexOutOfBounds: false,
	throwErrorIfLimitExceeded: false
}

DQueue.prototype.toString = function() {
	return "[object " + this.constructor.getName() + ", name: " + this.name + ", limit: " + this.config.limit + ", length: " + this.length() + "]";
}

DQueue.prototype.length = function() {
	return this.queue.length;
}

DQueue.prototype.add = function(value) {
	var cfg = this.config;
	if (isNaN(cfg.limit)) {
		this.queue.push(value);
	} else {
		if (this.length() < cfg.limit) {
			this.queue.push(value);
		} else {
			if (cfg.replaceLastIfLimitExceeded) { // if lifo then
				this.queue.splice(this.length() - 1, 1, value);
			}
			else if (cfg.throwErrorIfLimitExceeded) {
...
// Retrieves the element at index in the queue without removing it from the queue. If index not specified index will be set to 0 if fifo and length minus one if lifo.
// If index out bounds null is returned unless throwErrorIfIndexOutOfBounds is true.
DQueue.prototype.read = function(index) {
	var cfg = this.config;
	index = parseInt(index, 10);
	if (isNaN(index)) {
		index = cfg.fifo ? 0 : this.length() - 1;
	}
	if (index < 0 || index >= this.queue.length) {
		if (cfg.throwErrorIfIndexOutOfBounds) {
...
// empties the queue
DQueue.prototype.clear = function() {
	this.queue.length = 0;
	return this;
}

// get and remove
DQueue.prototype.get = function() {
	if (!this.length()) {
		return null;
	}
	return this.config.fifo ? this.queue.shift() : this.queue.pop();
}

function DTimer(config) {
	this.config = Object.configure(config, DTimer.defaultConfig);
	this.listeners = []; // array of listener objects which will be notified on time up
	this.timerId = NaN;
	this.startTime = NaN;
	this.endTime = NaN;
	this.duration = NaN;
...
DTimer.defaultConfig = {
	useRAF: false, // use requestAnimationFrame instead of setTimeout/setInterval to improve performance - See http://creativejs.com/resources/requestanimationframe/ and http://www.paulirish.com/2011/requestanimationframe-for-smart-animating/ and http://blog.teamtreehouse.com/efficient-animations-with-requestanimationframe
	delay: 17, // measured in milliseconds ~ 60 frames per second (1000 ms / 60 frames = 16.67 ms seconds per frame)
	initialDelay: NaN, // measured in milliseconds
	duration: Number.POSITIVE_INFINITY, // measured in milliseconds - Number.POSITIVE_INFINITY means that the timer will keep running until you manually stop it!
...
DTimer.prototype.start = function() {
	if (!this.isRunning()) {
		if (this.fireHandler("onBeforeStart")) {
			this.duration = Number(this.config.duration);
			dLib.assert(this.duration > 0, 'Duration must be a non-negative integer!');
			this.delay = parseInt(this.config.delay, 10);
			dLib.assert(this.delay > 0, 'Delay must be a positive integer!');
			this.initialDelay = parseInt(this.config.initialDelay, 10) || this.delay;
			this.endTime = NaN;
			this.startTime = performance.now();
			this.schedule(this.initialDelay);
		}
		this.fireHandler("onAfterStart");
	}
	return this;
}

DTimer.prototype.run = function(timestamp) {
	if (!this.isExecuting) {
		try {
			this.isExecuting = true;
			this.runningTime += this.runningTime == 0 ? this.initialDelay : this.delay;
			if (this.config.useRAF) {
				...
				if (timestamp - this.startTime > this.duration) {
					...
					if (this.keepRunning()) { // TODO: Now we draw/render before scheduling (requesting next animation frame) which is not good for browsers using setTimeout fallback
						// TODO: we should check if duration or delay is specified in order to aim for a lower frame rate than 60
						this.schedule();
					} else {
						this.stop();
					}
				...
				if (this.runningTime >= this.duration) {
					this.timeup().stop(); // must call timeup before stop
				} else {
					if (this.keepRunning()) {
						if (this.duration) {
							this.schedule(Math.min(this.delay, this.duration - this.runningTime));
						} else {
...
DTimer.prototype.schedule = function(delay) {
	if (this.config.useRAF) {
		...
		var d = Math.ceil(performance.now() - this.startTime - this.runningTime);
		this.timerId = setTimeout(this.run.bind(this), delay - d);
	}
	return this;
}
...
DTimer.prototype.pause = function() {
	if (this.isRunning()) {
		if (this.fireHandler("onBeforePause")) {
			var lap = {
				end: performance.now()
			}
			if (this.laps) {
				lap.start = this.laps[this.laps.length - 1].end;
			} else {
				this.laps = [];
				lap.start = this.startTime;
			}
			this.laps.push(lap);
			this.cancelTimer();
		}
		this.fireHandler("onAfterPause");
	}
	return this;
}
...
DTimer.prototype.resume = function() {
	if (!this.isRunning()) {
		if (this.fireHandler("onBeforeResume")) {
...
//		    this.cancelTimer();
		...
		this.fireHandler("onAfterResume");
	}
	return this;
}
...
DTimer.prototype.stop = function() {
	if (this.isRunning()) {
		if (this.fireHandler("onBeforeStop")) {
			this.endTime = performance.now();
			this.cancelTimer();
			this.runningTime = 0;
		}
		this.fireHandler("onAfterStop");
	}
	return this;
}

DTimer.prototype.cancelTimer = function() {
...
DTimer.prototype.getElapsedTime = function(exact) {
	if (!this.isRunning()) {
		return 0;
	}
	if (exact) {
...
DTimer.prototype.getRemainingTime = function(exact) {
	if (!this.isRunning()) {
		return 0;
	}
	if (exact) {
		return this.duration - (performance.now() - this.startTime);
	}
	return this.duration - this.runningTime;
}

DTimer.prototype.setRemainingTime = function(remainingTime) {
	if (this.isRunning()) {
		this.duration = this.runningTime + remainingTime;
	}
	return this;
}
...
(function() {
	...
	window.performance.now = window.performance.now || window.performance.oNow || window.performance.mozNow || window.performance.webkitNow || window.performance.msNow || function() {
	...
	window.requestAnimationFrame = window.requestAnimationFrame || window.webkitRequestAnimationFrame || window.mozRequestAnimationFrame || window.oRequestAnimationFrame || window.msRequestAnimationFrame || function(callback) {
		return window.setTimeout(callback, 1000 / 60); // 60 frames per second
	};
	window.cancelAnimationFrame = window.cancelAnimationFrame || window.webkitCancelAnimationFrame || window.mozCancelAnimationFrame || window.oCancelAnimationFrame || window.msCancelAnimationFrame || function(timerId) {
		window.clearTimeout(timerId);
	};
	window.indexedDB = window.indexedDB || window.mozIndexedDB || window.webkitIndexedDB || window.msIndexedDB;
	// We may need references to some window.IDB* objects (Mozilla has never prefixed these objects, so we don't need window.mozIDB*)
