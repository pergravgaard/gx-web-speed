...
DTimerButton.destroyAll = function() {
    for (var id in DTimerButton.instances) {
        DTimerButton.instances[id].destroy(false);
        delete DTimerButton.instances[id];
    }
}

DTimerButton.prototype.fireHandler = dLib.util.fireHandler;

DTimerButton.prototype.destroy = function(remove) {
    if (this.initialized) {
        if (this.isRunning()) {
            this.stop();
        }
        this.dElement.removeClass('timer-button');
        if (this.config.bindEvents) {
            this.dElement.off(this.config.bindEvents, this.onClickListener);
        }
        this.dProgress.destroy(remove);
        if (typeof remove != 'boolean' || remove) {
            var iid;
            for (var id in DTimerButton.instances) {
                if (DTimerButton.instances[id] === this) {
                    iid = id;
                    break;
                }
            }
            if (iid) {
                delete DTimerButton.instances[iid];
            }
        }
    }
}
...
DTimerButton.prototype.init = function() {
    if (!this.initialized) {
        ...
        this.dElement.addClass('timer-button');
...
DTimerButton.prototype.toString = function() {
    return "[object " + this.constructor.getName() + "] " + ((this.dElement && this.dElement.element) ? this.dElement.element.id : this.dElement) + ', running: '+ this.isRunning();
}
...
DTimerButton.prototype.start = function() {
    if (this.fireHandler('onBeforeStart', [].addAll(arguments))) {
        ...
        this.dElement.attr('data-disabled', true).addClass('running');
        this.dProgress.start();
        this.dTimer.start();
    }
    this.fireHandler('onAfterStart', [].addAll(arguments));
    return this;
}
...
DTimerButton.prototype.stop = function() {
    if (this.fireHandler('onBeforeStop', [].addAll(arguments))) {
        if (this.clickListener) {
            q('.progress-overlay, .progress-pie-container', this.dElement).off('click', this.clickListener).hide();
        }
        this.dElement.attr('data-disabled', false).removeClass('running');
        this.dProgress.stop();
        this.dTimer.stop();
        this.fireHandler('onAfterStop', [].addAll(arguments));
    }
    return this;
}
