...
window.dScreen = window.dScreen || {};
Object.extend(dScreen, {
    ...
    resolveViewport: function(dimension, targetWidth, targetHeight) {
        ...
        var content = '';
        var addMinMax = true;
        // Specify the width/height in CSS pixels that the device actually has and then the initial scale as the ratio between desired width/height in CSS pixels and actual width/height in CSS pixels.
        ...
        if (scale) {
            if (this.isAndroidWebview) { // works on Android 4.4.4, 4.1.2, but not 4.0.4
//                out('w: ' + targetWidth + ', h: ' + targetHeight + ', scale: ' + (scale).toFixed(4));
//                out('sw: ' + screen.width + ', sh: ' + screen.height);
                if (scale != 1) {
                    document.body.style.zoom = scale;
                    //document.body.style.webkitTransform = 'scale(' + scale + ')'; // seems to move offset as well - page is centered
                }
            } else {
...
(function() {
	...
	if ('onorientationchange' in window) {
		addEvent('orientationchange', function(e) { // not supported by Firefox
			dScreen.info.landscape = !dScreen.info.landscape; // One might think that if you tilt the screen 180 degrees this would fail, but the orientationchange event is not fired, if tilted 180 degrees
