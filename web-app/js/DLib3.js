...
//	},
	...
	event: {
		...
		resolveEvent: function(el) {
		    // The el argument may not refer to an element in this window! The following code allows execution in another window
		    var win = DElement.getDefaultView(el) || window;
		    return win ? win.event : null;
		},
		
		addEvents: function(target, eventTypes, handler, doCapture) {
			var types = eventTypes.trim().split(/\s+/);
			for (var i = 0, l = types.length; i < l; i++) {
				this.add(target, types[i], handler, doCapture);
			}
		},

        removeEvents: function(target, eventTypes, handler, doCapture) {
            var types = eventTypes.trim().split(/\s+/);
            for (var i = 0, l = types.length; i < l; i++) {
                this.remove(target, types[i], handler, doCapture);
            }
        },

		addOne: function(target, eventTypes, handler, doCapture) {
			var types = eventTypes.trim().split(/\s+/);
			var oneHandler = this.oneHandler(target, types, handler, doCapture);
			for (var i = 0, l = types.length; i < l; i++) {
				this.add(target, types[i], oneHandler, doCapture);
			}
		},
		
		/*	Adds one listener for several event types, but after first trigger other event types than the one triggered are removed again.
		 	Useful for handling pointer events across different browsers. */
		addX: function(target, eventTypes, handler, doCapture) {
			var types = eventTypes.trim().split(/\s+/);
			var exHandler = this.xHandler(target, types, handler, doCapture);
			for (var i = 0, l = types.length; i < l; i++) {
				this.add(target, types[i], exHandler, doCapture);
			}
		},

        // a wrapper handler for several event types - on first invocation all other event types will be removed again
		xHandler: function(target, eTypes, handler, doCapture) {
			var f = function(e) {
				for (var i = 0, l = eTypes.length; i < l; i++) {
					var t = eTypes[i];
					if (t === e.type) {
						handler.apply(this, arguments);
						dLib.event.add(target, t, handler, doCapture);
					} else {
                        dLib.event.remove(target, t, handler, doCapture);
                    }
				}
			};
            f.handler = handler; // will be used by the add method to save a reference to the added event listener for possible removal later
            f.bound = handler.bound;
            return f;
		},

        // a handler wrapper which will remove the handler on first invocation
		oneHandler: function(target, eTypes, handler, doCapture) {
			var f = function(e) {
				for (var i = 0, l = eTypes.length; i < l; i++) {
					var t = eTypes[i];
					if (t === e.type) {
	...
	/**	Cross-browser addEventListener method (uses Window/Document/Element.attachEvent in IE).
		...
		Note that this method returns this possibly wrapped handler/listener for possible removal later.
		...
		add: function(target, eventType, handler, doCapture) {
            ...
            // the handler argument may be a wrapper on which the original handler is stored in the handler property: handler.handler || handler
			var obj = { target: target, origHandler: handler.handler || handler, eventType: eventType, doCapture: doCapture };
		/*	It is necessary in IE6 with SP1 to change the target from self to window.
			...
			if (target.addEventListener) {
                //out('adding: ' + eventType + ' - ' + (target.id || target.tagName || target));
				target.addEventListener(eventType, handler, doCapture);
			}
		...
		remove: function(target, eventType, handler, doCapture) {
			...
			if (handlerToRemove) {
//                out('removing: ' + eventType + ' - ' + (target.id || target.tagName || target));
	...
	/**	Cross-browser method to add an onDOMReady listener.
		...
		addDOMReady: function(listener) {
            if (document.addEventListener) {
                return dLib.event.add(document, "DOMContentLoaded", listener);
            }
...
(function() {
	...
	if (!isNative(Function.prototype, "bindAsEventListener")) {
		...
		Function.prototype.bindAsEventListener = function(targetObj) {
			...
			var fn = this;
			var f = function(e) {
                                e = e || dLib.event.resolveEvent(targetObj);
...
DEvent.isTouchSupported = function() {
...
DDocument.prototype.on = function(eventTypes, handler, doCapture) {
    dLib.event.addEvents(this.document, eventTypes, handler, doCapture);
	return this;
}

DDocument.prototype.one = function(eventTypes, handler, doCapture) {
    dLib.event.addOne(this.document, eventTypes, handler, doCapture);
    return this;
}

DDocument.prototype.xon = function(eventTypes, handler, doCapture) {
    dLib.event.addX(this.document, eventTypes, handler, doCapture);
    return this;
}
...
DDocument.prototype.off = function(eventTypes, handler, doCapture) {
    dLib.event.removeEvents(this.document, eventTypes, handler, doCapture);
	return this;
}
...
DElement.prototype.on = function(eventTypes, handler, doCapture) {
    dLib.event.addEvents(this.element, eventTypes, handler, doCapture);
	return this;
}

DElement.prototype.one = function(eventTypes, handler, doCapture) {
    dLib.event.addOne(this.element, eventTypes, handler, doCapture);
    return this;
}

DElement.prototype.xon = function(eventTypes, handler, doCapture) {
    dLib.event.addX(this.element, eventTypes, handler, doCapture);
    return this;
}
...
DElement.prototype.off = function(eventTypes, handler, doCapture) {
    dLib.event.removeEvents(this.element, eventTypes, handler, doCapture);
    return this;
}
...
DElement.prototype.css = function(property, value) {
	var rv = DElement.css(this.element, property, value);
...
DElementList.prototype.on = function(eventTypes, handler, doCapture) {
	this.forEach(function(entry) {
		dLib.event.addEvents(entry, eventTypes, handler, doCapture);
	});
	return this;
}

DElementList.prototype.one = function(eventTypes, handler, doCapture) {
	this.forEach(function(entry) {
		dLib.event.addOne(entry, eventTypes, handler, doCapture);
	});
	return this;
}

DElementList.prototype.xon = function(eventTypes, handler, doCapture) {
	this.forEach(function(entry) {
		dLib.event.addX(entry, eventTypes, handler, doCapture);
	});
	return this;
}
...
DElementList.prototype.off = function(eventTypes, handler, doCapture) {
    this.forEach(function(entry) {
        dLib.event.removeEvents(entry, eventTypes, handler, doCapture);
    });
	return this;
}
