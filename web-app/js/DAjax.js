...
function HttpRequest(config) {
	...
	this.crossDomain = !!this.config.crossDomain;
	this.async = this.config.async;
	this.url = ''; // is resolved in the open method
...
HttpRequest.success = function(xhr, loc) {
	...
	try {
		...
		var s = xhr.status;
		return (!s && loc.protocol.toLowerCase().indexOf("file") == 0) || (s >= 200 && s < 300) || s === 304 || s === 1223 || (s === 0 && window.opera) || (dLib.ua.isSafari && s === undefined);
	}
	catch (err) {
	}
	return false;
...
HttpRequest.prototype.timeout = function() {
	...
	this.fireHandler("onTimeout", arguments);
	return this;
}
...
HttpRequest.prototype.toggleProgress = function(show) {
	...
	if (cfg.progressId && cfg.progressClass) {
		...
		if (el) {
			if (show) {
				DElement.addClass(el, cfg.progressClass);
			} else {
				DElement.removeClass(el, cfg.progressClass);
			}
		}
	}
	return this;
...
HttpRequest.prototype.abort = function(fire) {
	if (this.request && !this.aborted && !this.hasCompleted) {
		...
		if (typeof fire != "boolean" || fire) {
			this.fireHandler("onAbort", arguments);
		}
	}
	return this;
}
...
HttpRequest.prototype.open = function(method, url, async, username, password) {
	...
	this.resolveUrl(url || this.config.url); // may alter crossDomain, async and method
	...
	this.aborted = false;
	this.hasCompleted = false;
	if (this.fireHandler("onBeforeOpen", arguments)) {
	...
	this.fireHandler("onAfterOpen", arguments);
	return this;
}
...
HttpRequest.prototype.send = function(params, doCache) {
	if ((this.request || this.crossDomain) && this.url) {
		...
		var parameters = this.resolveParameters(params);
		if (!this.crossDomain) {
			this.setRequestHeaders(window.FormData && parameters instanceof window.FormData);
		}
		var args = [parameters].addAll(arguments);
		if (this.fireHandler("onBeforeSend", args)) {
		...
		this.fireHandler("onAfterSend", args);
	}
	return this;
}

HttpRequest.prototype.sendCrossDomainRequest = function(parameters) {
	var callbackName = this.config.callbackName || 'crossDomainCallback' + new Date().getTime();
...
HttpRequest.prototype.onreadystatechange = function(e) {
	if (this.request.readyState < 4) {
		this.fireHandler("onNotComplete", arguments);
	}
		...
		if (!this.aborted) {
			...
			this.fireHandler("onComplete", arguments);
			this.fireHandler(this.success() ? "onCompleteOk" : "onCompleteNotOk", arguments);
		}
	}
}
	...
	if (this.crossDomain && response) {
		switch (contentType) {
			...
			case "application/xml":
				...
				if (this.xmlDocument.document) {
					this.xmlDocument.document = response;
		...
		switch (contentType) {
			...
			case "text/html":
			default:
				if (this.request.responseType == 'arraybuffer') {
...
HttpRequest.prototype.resolveUrl = function(url) {
	...
	if (typeof url == 'string') {
		...
		if (["post"].contains(this.method)) {
			...
			if (a.length == 1 && this.config.data) {
				if (typeof this.config.data == 'string') {
					url += '?' + this.config.data;
				}
				else if (Object.isObject(this.config.data)) {
					url += '?' + Object.serialize(this.config.data);
				}
			}
			this.url = url;
		}
