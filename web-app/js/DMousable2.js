...
//	endEvent: 'mouseup touchend',
    startEvent: 'x-pointerdown',
    moveEvent: 'x-pointermove',
    endEvent: 'x-pointerup',
	dElementConfig: {
...
DMousable.prototype.bindEvents = function() {
	var f = this.getCaptureListener();
	if (typeof f == "function") {
		var cfg = this.config;
		for (var i = 0, l = this.captureDElements.length; i < l; i++) {
			var captureDElement = this.captureDElements[i].on(cfg.startEvent, f);
			if (cfg.cursor) {
...
DDragable.prototype.dragStart = function(e) {
	...
	if (this.dragTarget || (e.isMouseEvent() && !e.isLeftButtonDown()) || (e.isTouchEvent() && !e.isOneTouchEvent())) {
		return;
	}
	if (this.fireHandler("onBeforeDragStart", [e])) {
		...
		e.preventDefault().stopPropagation(); // to prevent default here, prevents possible native dragstart and mousedown event from firing, but in webkit browsers and IE 10 dragstart and mousedown cannot coexists!!
		var cfg = this.config;
		this.dElement.addClass(cfg.dragClass).css("zIndex", DMousable.stackOrder++);
		// start catching pointer coordinates
		this.dragOffset = [e.getPageX() - parseInt(this.style.left, 10), e.getPageY() - parseInt(this.style.top, 10)];
		var doc = this.dElement.getOwnerDocument();
		dLib.event.deleteAll(doc, cfg.moveEvent, true);
		dLib.event.deleteAll(doc, cfg.endEvent, true);
//		this.mousemoveHandler = this.drag.bindAsEventListener(this);
//		this.pointerupHandler = this.dragEnd.bindAsEventListener(this);
//		var met = e.isTouchEvent() ? cfg.moveEvent.replace('mousemove').trim() : cfg.moveEvent; // do not wanna fire both mousemove and touchmove - can cause undesirable results (fx Safari in iOS6)
//		this.dragTarget = g(doc).on(met, this.mousemoveHandler, true).on(cfg.endEvent, this.pointerupHandler, true);
		this.dragTarget = g(doc);
		this.pointermoveHandler = dLib.event.addAll(doc, cfg.moveEvent, this.drag.bindAsEventListener(this), true);
        this.pointerupHandler = dLib.event.addAll(doc, cfg.endEvent, this.dragEnd.bindAsEventListener(this), true);
	}
...
DDragable.prototype.dragEnd = function(e) {
	if (this.fireHandler("onBeforeDragEnd", [e])) {
		...
		var cfg = this.config;
		this.dragTarget.off(cfg.moveEvent, this.pointermoveHandler, true).off(cfg.endEvent, this.pointerupHandler, true);
...
DResizable.prototype.captureResize = function(e) {
	...
	if (this.fireHandler("onBeforeCaptureResize", [e])) {
		...
		this.pointermoveHandler = this.resize.bindAsEventListener(this);
		this.pointerupHandler = this.releaseResize.bindAsEventListener(this);
		var met = e.isTouchEvent() ? cfg.moveEvent.replace('mousemove').trim() : cfg.moveEvent;
		this.resizeTarget = g(this.dElement.getOwnerDocument()).on(met, this.pointermoveHandler, true).on(cfg.endEvent, this.pointerupHandler, true);
	}
...
DResizable.prototype.releaseResize = function(e) {
	if (this.fireHandler("onBeforeReleaseResize", [e])) {
		var cfg = this.config;
		this.resizeTarget.off(cfg.moveEvent, this.pointermoveHandler, true).off(cfg.endEvent, this.pointerupHandler, true);
...
DHoverable.defaultConfig = Object.configure({
	position: 'absolute',
	useMouseOffset: true,
	startEvent: 'mouseover touchstart',
	endEvent: 'mouseout touchend',
...
DHoverable.prototype.bindEvents = function() {
	...
	var cfg = this.config;
	for (var i = 0, l = this.captureDElements.length; i < l; i++) {
		this.captureDElements[i].on(cfg.startEvent, fnOver).on(cfg.endEvent, fnOut);
	}
	this.dElement.on(cfg.startEvent, this.cancelTimeout.bind(this)).on(cfg.endEvent, fnOut);
	return this;
}
...
DHoverable.prototype.addCaptureElement = function(element) {
	...
	var fnOver = this.hover.bindAsEventListener(this);
	var fnOut = this.releaseHover.bindAsEventListener(this);
	dElement.on(this.config.startEvent, fnOver).on(this.config.endEvent, fnOut);
	return this;
}
...
DHoverable.prototype.calcHoverPosition = function(e, offsetElement) {
	...
	var target = offsetElement || e.getTarget().element;
...
DHoverable.prototype.hide = function() {
	return this.dElement.hide().moveTo([0, 0]);
}
...
DZoomable.prototype.init = function() {
	if (!this.initialized) {
		var dElement = g(document.createElement("div"));
		dElement.element.id = "divZoom" + new Date().getTime();
		dElement.setClass(this.config.zoomClass);
...
DZoomable.prototype.captureZoom = function(e) {
	...
	if (this.fireHandler("onBeforeCaptureZoom", [e])) {
		...
		this.pointermoveHandler = this.zoom.bindAsEventListener(this);
		this.pointerupHandler = this.releaseZoom.bindAsEventListener(this);
		this.moveTarget = g(this.dElement.getOwnerDocument()).on("mousemove touchmove", this.pointermoveHandler, true).on("mouseup touchend", this.pointerupHandler, true);
	}
...
DZoomable.prototype.releaseZoom = function(e) {
	if (this.fireHandler("onBeforeReleaseZoom", [e])) {
		...
		this.moveTarget.off("mousemove touchmove", this.pointermoveHandler, true).off("mouseup touchend", this.pointerupHandler, true);
	}
