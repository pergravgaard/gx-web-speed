...
(function() {
	window.Log = {
		...
		log: function(msg, methodName) {
			...
			if (window.console) {
				if (methodName in window.console) {
					window.console[methodName](msg);
				}
				else if ('log' in window.console) { // IE10 goes here
	...
	// now you can write Log.d('some message'), Log.i('some message') etc.
})();
...
var dBugger = {
	...
	styleString: 'width:30%;bottom:10px;overflow:scroll;border:black solid 1px;position:fixed;right:10px;top:10px;z-index:29999;padding:3px;font-family:verdana;font-size:8pt;opacity:.8;background-color:#fff;color:#000;filter:alpha(opacity=80);',
	...
	getMonitorHTML: function(styleString, className) {
		...
		return '<div id="divMonitor" style="' + s + '" class="' + className + '" onpointerdown="if (event.pointerType != \'mouse\') { this.innerHTML=\'\'; }" onMSPointerDown="if (event.pointerType != 4) { this.innerHTML=\'\'; }" ontouchstart="this.innerHTML=\'\';" ondblclick="this.innerHTML=\'\';" title="double click to clear"><\/div>';
	},
	...
	showError: function(msg, url, line) {
		...
		if (typeof msg == "string") {
			...
			if (document.readyState == 'complete' || document.readyState == 'interactive') {
				...
				alert(txt); // it does not make sense to log the error to console as this is automatically done by the browser
			}
		}
		return false;
	},
...
(function() {
	if (window.addEventListener && dBugger.closePopupsOnLeave) {
